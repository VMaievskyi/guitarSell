...
func (h *DBHandler) guitarsEdit(rw http.ResponseWriter, req *http.Request, id int64) {
	...
	//file, err := os.Create(fmt.Sprintf("~/Documents/%s", header.Filename)) // we would normally need to generate unique filenames.
	//if err != nil {
	...
	guitar := Guitar{Id: id, Name: guitarForm.Name, Brand: guitarForm.Brand, Year: guitarForm.Year,
		Price: guitarForm.Price, Color: guitarForm.Color, ImageUrl: "", CreatedAt: time.Now(), UpdatedAt: time.Now()}
	h.db.Save(&guitar)
	h.r.JSON(rw, http.StatusOK, &guitar)
}
